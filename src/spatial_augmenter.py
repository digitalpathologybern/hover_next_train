...
class SpatialAugmenter(torch.nn.Module,):
    ...
    def mirror(self, img, label, random_state=None):
        ...
        theta = torch.tensor([[[x  , 0.0, 0.0],
                               [0.0, y  , 0.0]]], device=self.device)
        grid = F.affine_grid(theta.repeat(img.size()[0],1,1), img.size(), align_corners=False)
        if label is not None:
            return F.grid_sample(img, grid, mode=self.interpolation, padding_mode=self.padding_mode, align_corners=False), F.grid_sample(label, grid, mode='nearest', padding_mode=self.padding_mode, align_corners=False)
        else:
            return F.grid_sample(img, grid, mode=self.interpolation, padding_mode=self.padding_mode, align_corners=False), None
    ...
    def zoom(self, img, label, random_state=None):
        ...
        theta = torch.tensor([[[zoom_factor  , 0.0, 0.0],
                               [0.0, zoom_factor  , 0.0]]], device=self.device)
        grid = F.affine_grid(theta.repeat(img.size()[0],1,1), img.size(), align_corners=False)
        if label is not None:
            return F.grid_sample(img, grid, mode=self.interpolation, padding_mode=self.padding_mode, align_corners=False), F.grid_sample(label, grid, mode='nearest', padding_mode=self.padding_mode, align_corners=False)
        else:
    ...
    def rotate(self, img, label, random_state=None):
        ...
        theta = torch.tensor([[[math.cos(rad),  -math.sin(rad), 0.0],
                               [math.sin(rad),   math.cos(rad), 0.0]]], device=self.device,dtype=torch.float)
        grid = F.affine_grid(theta.repeat(img.size()[0],1,1), img.size(), align_corners=False)
        if label is not None:
            return F.grid_sample(img, grid, mode=self.interpolation, padding_mode=self.padding_mode, align_corners=False), F.grid_sample(label, grid, mode='nearest', padding_mode=self.padding_mode, align_corners=False)
        else:
    ...
    def identity_grid(self, img):
        ...
        return F.affine_grid(theta.repeat(img.size()[0],1,1), img.size(), align_corners=False)
    ...
    def create_elastic_transformation(self, shape, alpha = [80,80], sigma = 8):
        ...
        dx = blur(torch.rand(*shape,device=self.device).unsqueeze(0).unsqueeze(0)*2 -1) * alpha[0]/shape[0]
        dy = blur(torch.rand(*shape,device=self.device).unsqueeze(0).unsqueeze(0)*2 -1) * alpha[1]/shape[1]
